...
The public npm handoff contains five version-aligned packages. `scripts/pack-public-developer-candidate.mjs` builds them from the tested source, replaces local `file:` links with exact `0.1.0` dependencies, removes the private flag and injects the common verified repository metadata. It does not publish.

| Package | Purpose | Publish order |
|---|---|---:|
| `@opace/content-integrity-contracts` | Frozen types, constants and semantic contract | 1 |
| `@opace/content-integrity-core` | Deterministic inspection, diffs and receipts | 2 |
| `@opace/content-integrity-browser` | DOM projection and Worker adapter | 3 |
| `@opace/content-integrity-client` | Typed loopback API client | 4 |
| `@opace/content-integrity-cli` | Node command line; binary `opace-integrity` | 5 |

All packages use MIT, Node `>=20`, public access, the Opace author identity and the target monorepo/issue URLs. npm requires each package to exist before OIDC trusted publishing can be configured, so the first `0.1.0` release uses interactive account 2FA and has no CI provenance. Every later release must use stage-only OIDC, provenance and maintainer 2FA approval. Package-specific descriptions, keywords, exports, types, files and binary fields remain in their source manifests. The CLI homepage points to the intended CLI route; the library packages point to the parent product route.
