...
test("a model outside its reliable length range publishes no reading at all", () => {
  ...
  // The runtime boundary uses the current Cycle 5, held-out 100-word cell. Its
  // denominator and small-cell warning must travel with the figure so a stronger
  // result is not mistaken for permission to publish a probability on short text.
  assert.ok(v.limitations.some((l) => /43 of 56/.test(l) && /76\.8%/.test(l) && /cell is small/.test(l)),
    `the short-text limitation must carry the current held-out figure, denominator and small-cell warning: ${v.limitations.join(" | ")}`);
});
