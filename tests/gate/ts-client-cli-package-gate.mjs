...
const names=["contracts","core","client","cli"].map(name=>`opace-content-integrity-${name}-0.0.0-private.tgz`);
...
try{
  ...
  const dependencies=Object.fromEntries(["contracts","core","client","cli"].map((name,index)=>[`@opace/content-integrity-${name}`,`file:${join(first,names[index])}`]));
  ...
  writeFileSync(join(consumer,"probe.ts"),'import {LocalClient,parseSse} from "@opace/content-integrity-client";import {inspect} from "@opace/content-integrity-core";import {runCli} from "@opace/content-integrity-cli";void (null as unknown as LocalClient);void parseSse;void inspect;void runCli;\n');
  ...
  run(process.execPath,["--input-type=module","-e",'const contracts=await import("@opace/content-integrity-contracts");const core=await import("@opace/content-integrity-core");const client=await import("@opace/content-integrity-client");const cli=await import("@opace/content-integrity-cli");if(contracts.SCHEMA_VERSION!=="1.0"||typeof core.inspect!=="function"||typeof client.createLocalClient!=="function"||typeof cli.runCli!=="function")throw Error("missing export")'],consumer);
  const bin=join(consumer,"node_modules/.bin/opace-integrity");assert.match(run(bin,["--help"],consumer,true),/Usage: opace-integrity/);const inspected=run(bin,["--format","json","inspect","-"],consumer);assert.equal(JSON.parse(inspected).schema_version,"1.0");
