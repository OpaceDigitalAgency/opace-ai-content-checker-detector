...
Status: 1.0.5 (30 August 2026) rebuilds the bundled engine from the current core and supersedes 1.0.4; the FINAL1 exact-package matrix and independent review were passed by 1.0.4 and must be re-run  
**Blocking:** the refreshed `assets/js/core.mjs` no longer agrees with the plugin's own PHP analyser in `includes/Analysis/`. 4 of 9 tests in `tests/js/cross-runtime-parity.test.mjs` fail and are left failing deliberately: the two implementations declare different rule versions (`unicode:2026.08.2`/`en-signals:2026.08.6` against `unicode:2026.08.1`/`en-gb:2026.08.1`) and the JS side now runs 116 writing rules where PHP runs 3, so the browser Lab and the REST-backed editor sidebars report different findings for the same text. Aligning the PHP constants alone would hide the divergence rather than fix it.  
...
The sole current candidate is `dist/opace-ai-content-integrity-1.0.5.zip`, SHA-256 `c4431c483f7152d058a7624d518959f3d728137cb4f2a83436b554c72c53f0bd`, built 30 August 2026 against the current core. **Its gate evidence is open.** Everything recorded in the rest of this paragraph belongs to the superseded 1.0.4 ZIP (`084556a727022f23cd33e6b8111694fb6e447898d9c5b005b091d2057f8520ec`), whose executable bytes 1.0.5 does not share: `assets/js/core.mjs` is rebuilt and the version strings move to 1.0.5. Plugin Check, the official readme validator, the file inventory and the runtime evidence must all be re-run against 1.0.5 before submission. The 27 August documentation refreeze supersedes `a69f0eac...e3a4e`; only `README.md` and `readme.txt` differ, so the installed executable/runtime tree is unchanged. Three successful ordinary builder runs are byte-identical. The exact refrozen ZIP has 301 files and 258 PHP files; Plugin Check 2.1.0 reports no errors and the official readme validator reports no errors or warnings. The unchanged runtime bytes retain the minimum/current/Multisite, 301/301 parity and PHP 7.4 evidence.
