...
test("printable report is the shared branded report with the complete evidence baseline",()=>{const fixture=canonicalFixture(),html=render(fixture,"html"),visible=visibleReport(html),words=textOnly(html);
  ...
  assert.match(words,/highest model score is in section 2 of 2/);
  ...
  assert.match(words,/How this section was scored/);assert.match(words,/segments-v3, raw-v1, features-v1, margin-v1/);
  ...
  assert.match(words,/Opace Cycle-5 AI-pattern model/);assert.match(words,/detector\.cycle5/);assert.match(words,/Reading complete/);assert.match(words,/Review writing patterns/);
