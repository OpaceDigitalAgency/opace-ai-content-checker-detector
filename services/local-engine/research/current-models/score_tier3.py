...
paths are parameterised, so the raw and stripped runs share one code path.
  ...
  python score_tier3.py current-eval-set.jsonl tier3-raw.jsonl
"""
...
def main() -> None:
    ...
    for i, d in enumerate(rows, 1):
        ...
        out.append({"id": d["id"], "provider": d.get("provider"), "era": d.get("era"),
                    ...
                    "genre": d.get("genre"), "register": d.get("register"),
                    "model_release": d.get("model_release"), "source": d.get("source"),
                    "tier3_int8pc": round(p, 8), "flagged_0857": p >= THRESHOLD})
        if i % 200 == 0:
