"""Mongabay -> long-form environmental journalism (human side).

A second newsroom, so the journalism register is not one publication's house
style. Mongabay publishes reported environmental features under CC BY-ND 4.0;
its republishing terms assert no restriction on AI or research use, and the
text is stored verbatim, which ND permits.
...
API = "https://news.mongabay.com/wp-json/wp/v2/posts"
OUT = os.path.join(common.RAW, "mongabay.jsonl")
...
def to_row(r: dict) -> dict | None:
    ...
    return {
        ...
        "era_year": year,
        "discipline": "environment-reporting",
        "publisher": "Mongabay",
