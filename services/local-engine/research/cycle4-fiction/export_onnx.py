...
CKPT = os.environ.get("C4_CKPT") or os.path.join(HERE, f"ckpt-{ARM}")
FP32 = os.path.join(MODELS, f"tier3-cycle4-{ARM}-e5small-fp32.onnx")
INT8 = os.path.join(MODELS, f"tier3-cycle4-{ARM}-e5small-int8-perchannel.onnx")
PROTECTED = {os.path.join(MODELS, "tier3-e5small-int8-perchannel.onnx"),
             ...
             os.path.join(MODELS, "tier3-cycle3-e5small-fp32.onnx"),
             os.path.join(MODELS, "tier3-cycle3-e5small-int8-perchannel.onnx")}
MAX_LEN = 512
...
def main() -> None:
    ...
    cal = [json.loads(l) for l in open(data)
    ...
    json.dump(out, open(os.path.join(HERE, f"onnx-export-report-{ARM}.json"), "w"), indent=2)
