"""CYCLE-4 fine-tune: the cycle-3 recipe, parameterised so the arms are
comparable.

Cycle 3 fixed length and repetition and broke fiction: human fiction false
positives went 23/260 -> 29/260 and all six new ones were genuine narrative
prose. The mechanism is in the corpus, not the optimiser - the training split
holds NO human fiction at all, and its 339 AI fiction rows sit entirely in the
test split, so nothing in training ever taught the model what fiction looks
like on either side. Cycle 3 then trained the model to stop reading lexical
repetition as evidence of a human, and fiction is lexically repetitive.

Two arms are run from this one file so the only difference between them is the
thing being tested:

  A  --arm rebalance  cycle-3 data exactly, but the repetition half of the
     hard-negative boost removed. Tests whether the regression can be undone by
     weighting alone, with no new data and no spend.
  B  --arm cycle4     cycle-3 data plus matched new fiction on BOTH sides, and
     a `creative` register cell of its own.
...
DATA = os.environ.get("C4_DATA") or os.path.join(HERE, "dataset.jsonl")
START = os.path.join(RESEARCH, "cycle2-train", "cycle2-checkpoint")
CKPT = os.environ.get("C4_CKPT") or os.path.join(HERE, "cycle4-checkpoint")
MAX_LEN = 512
EPOCHS = 3
LR = 1e-5
BATCH = 16
...
def assign_weights(rows):
    ...
    for r in rows:
        ...
        hn = r.get("hard_negative")
        if ARM == "rebalance" and axis_of(r) == "shortform":
            # drop the repetition half of the flag, keep the length half
            hn = bool(r.get("target_len", 999) <= 300)
        if hn:
            r["_w"] *= HARD_BOOST
...
def main():
    ...
    json.dump(rep, open(os.path.join(
        HERE, f"train-report-{ARM}.json"), "w"), indent=2)
