...
def parser():
    ...
    inspect_command = commands.add_parser("inspect");inspect_command.add_argument("path", nargs="?", default="-");inspect_command.add_argument("--receipt");inspect_command.add_argument("--checks");inspect_command.add_argument("--locale",default="en-GB");inspect_command.add_argument("--fail-on",choices=("attention","fail","gate"));inspect_command.add_argument("--fail-on-gate",action="store_true")
    ...
    serve_command = commands.add_parser("serve");serve_command.add_argument("--host", default="127.0.0.1");serve_command.add_argument("--port", type=int, default=8741)
...
def run(argv=None) -> int:
    ...
    if args.command == "serve":
        ...
        run_token,admin_token=take_service_tokens_from_environment();serve(args.host,args.port,run_token,admin_token);return 0
    ...
    if args.command == "inspect":
        ...
        validate("analysis-request.schema.json", request);result = inspect(request);validate("analysis-result.schema.json", result)
        if args.receipt:
...
def main(argv=None):
    try: code = run(argv)
        ...
        message=str(error);print(message,file=sys.stderr);code=3 if "not_configured" in message or "unsupported" in message else 10
    except Exception:
