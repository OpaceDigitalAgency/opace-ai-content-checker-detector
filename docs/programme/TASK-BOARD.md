...
Last updated: 1 September 2026, by the website agent after the Phase-5 cycle-5 site deployment.

## Done today — 1 September 2026

| Item | Evidence |
|---|---|
| **Cycle-5 server deployment (Phase 4, owner GO)**: `opace-detector-00010-4dt` serves `tier3-cycle5-full` fp32 (3-input) — feature extraction vendored verbatim into `reference-server/c5features/` (10/10 golden full vectors ≤1e-9, now a deploy gate via `test_c5features.py`); margin-space rule `max(m1, m2+0.34) >= 3.571` ported from analyse.py; raw-v1 input keyed to the model's own thresholds file (md-strip-v1 stays the cycle-2 behaviour); `/v1/health` advertises `model_build 45e00978b10d1df6 / raw-v1 / features-v1 / margin-v1` for the Phase-5 client gate. Local server-path seg_m reproduced the fitted `scores/c5-*.jsonl` within 2e-6 pre-deploy; live margins match to 6 dp. Both drills re-proven (kill switch 31.80 s, range 32–88 s over seven fires, ~4.2 s downtime, byte-exact restore; ten-path probe zero hits, canary-proven). End-to-end: AI long-form flagged, `govuk-fa21a585224e` clean on raw text (p 0.0343), 160-word AI flagged (short-form power). Cycle-2 weights retained in the image; rollback = env vars in `deploy.sh` | `.agent/docs/ai-content-integrity/CLOUD-RUN-SAFETY-REVERIFICATION-2026-09-01-C5.md` |
...
## Done — website agent batch 7, 1 September 2026 (site commits `dae4a18d`, `3f04c8e8`)

1. **Cycle-5 site deployment (Phase 5, owner GO)**: the browser route now runs `tier3-cycle5-full-e5small-int8-perchannel.onnx` (3-input: tokens, mask, 8 z-normalised structural features per segment, computed on raw text by the features-v1 TS port). Verdict decided in margin space — flag iff `max(m1, m2+0.34) >= 3.570935` — with the display ladder's top floor asserted equal to the exact sigmoid mapping of the comparator at load time (one comparator for ladder and flag). Input normalisation is a per-model registry property: cycle 5 declares `raw-v1`, md-strip-v1 and its offset mapping stay for models that declare it; sentence-evidence floors carry `fitted_for: tier3-cycle2-v1` and the sentence layer declares non-calibration on cycle-5 runs. Push gated on `/v1/health` advertising `45e00978b10d1df6 / raw-v1 / features-v1 / margin-v1` (passed).
2. **Browser runtime measured before it was claimed**: the int8 file through onnxruntime-web WASM (the engine build the site ships) over the full 5,558-document corpus + 610-document matched held-out slice — AI 900/922 (97.6%), human FP 73/4,636 (1.57%, materially above the fp32 route's 0.99% and printed per-route), fiction 8/227 (3.5%), matched pairs 161/192 and 1/418, route disagreement 37/5,558 (0.67%). Native-int8 proxy figures retired to `native_int8_reference`, labelled a different engine build. Bands re-derived from the browser score distribution.
3. **Production E2E, both routes**: 328-word AI draft FLAGGED (0.98 server / 0.98 browser); 109-word AI draft FLAGGED (0.97 both — the marginal fp32-3.67 sample held on int8); 3,234-word raw-markdown human page (18F handbook leave.md) CLEAN on both (Unclear, top section 0.89 fp32 / 0.87 int8, no flag) with syntax intact — no md-strip on cycle 5.
4. **Research pages measured on cycle 2 now say so**: `RETIRED.cycle2Pair` + `CYCLE2_DETECTOR` stamp 12 pages; cycle-2 refit narratives pin their own numbers instead of tracking `SHIPPED`; the 27% paper updated from "candidate, not deployed" to deployed-with-evidence (`3f04c8e8`), humaniser cell left open and unclaimed; disclosure derives training-cycle label and input-contract sentence from the active model.
