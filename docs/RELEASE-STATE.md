...
### The re-vendor exposed a second analysis implementation in the WordPress plugin

`wordpress/opace-ai-content-integrity/includes/Analysis/` reimplements the deterministic analysis
in 507 lines of PHP — its own Unicode table, homoglyph list, three `en-gb:2026.08.1` pattern rules
and protected-span extractor. It serves the REST route the Block and Classic editor sidebars use;
the admin Lab runs `assets/js/core.mjs` in the browser. `tests/js/cross-runtime-parity.test.mjs`
asserts the two agree exactly, and passed only because `core.mjs` was frozen at the PHP's vintage.
With the engine refreshed, **4 of that suite's 9 tests fail**. On its four fixtures the findings
still match; the declared versions do not — `unicode:2026.08.2` against `unicode:2026.08.1`,
`en-signals:2026.08.6` against `en-gb:2026.08.1` — and behind the labels the JS side now runs 116
writing rules where PHP runs 3, so the plugin's two routes report different findings for the same
text on any real document.

**Those tests are left failing on purpose.** Editing the PHP constants would align the labels while
the rule sets stayed apart, which is the worse failure. HANDOVER §11 states there must be no
parallel analysis implementations to drift apart; this one predates the re-vendor, which only made
it visible. **This is now the top blocker on the WordPress plugin**, ahead of the WordPress.org
submission mechanics, and closing it is a product decision: port the rules to PHP, route the editor
sidebars through the browser engine, or publish per route which rule set ran.
...
| WordPress plugin 1.0.5 | sole current exact local ZIP, built 30 August 2026 against the current core, SHA-256 `c4431c483f7152d058a7624d518959f3d728137cb4f2a83436b554c72c53f0bd`, superseding 1.0.4 `084556a727022f23cd33e6b8111694fb6e447898d9c5b005b091d2057f8520ec` | **open** — the implementer and independent package/runtime/submission-preparation reviews were passed by 1.0.4 and do not carry to 1.0.5 | **open** — `assets/js/core.mjs` and every version string changed, so deterministic three-build parity, Plugin Check, the official readme validator and the minimum/current/Multisite, PHP 7.4, lifecycle, REST, editor, focus, responsive and scoped axe evidence must all be re-run against the 1.0.5 ZIP | no | no | no | Confirmed unlisted 29 August 2026: `wordpress.org/plugins/opace-ai-content-integrity/` redirects to the directory search page. The 1.0.5 ZIP was built on 30 August 2026 with `bin/build-plugin.sh`; the copy under `dist/wordpress-submission-prep-1.0.4/plugin/` is the superseded 1.0.4 ZIP and that whole preparation bundle needs regenerating. The GitHub URL gate is now met, and `.wordpress-org/` banner and icon assets are committed; `readme.txt` carries the credits and the weakness list. WordPress.org readme does not render SVG, so charts must be linked rather than embedded. Per the owner's standing rule, always add cache busting and increment the version on any plugin change. **Blocking: the PHP/JS cross-runtime parity break described above — 4 of 9 tests in `tests/js/cross-runtime-parity.test.mjs` fail and are left failing on purpose.** Also remaining: a re-run of every 1.0.4 gate against the 1.0.5 ZIP, a fresh rules audit covering the 71 writing rules the audited build did not have, listing copy for them, three-build parity (the second and third builds could not be run — Docker Desktop stopped mid-run on 30 August), owner Safari/VoiceOver consent, WordPress.org account/slug and SVN submission, post-submission verification |
