...
control and current revision: budget `[budget id redacted from the public record]` is a configured £50
...
| Public npm/Astro/Python 0.1.0 packages | exact local candidates prepared; README refreeze complete 27 August | independent local submission audit plus documentation-refreeze verification passed | deterministic repeat, clean consumers/matrix, archive/public-tree hygiene, Twine, five developer npm dry-runs, audits, root G1/G2 regression and eight-hash verifier green | no | no | no | Confirmed unpublished 29 August 2026: npm returns 404 for `@opace/content-integrity-core`, `@opace/astro-content-integrity` and `@opace/watermark-lab`; PyPI returns 404 for `opace-content-integrity`. Exact current hashes in `submission-prep/submission-manifest.json`. The Astro and npm candidates were repacked twice on 30 August 2026, the second time after the rule-string rewrite and verified by extraction: Astro tarball `5999f32d…60f00e94` (was `0fd6716e…15b86d7a`, itself was `4a45e453…1e82da5`), npm five-package manifest `21a2eaad…5b46a3f70` (was `ae143295…aa373f5f17`, itself was `f2e09e2e…5193fe20`). The Python wheel `ddd0b160…d3b943d` and sdist `34c65f6e…40587f7d46` are unchanged and were **not** rebuilt: extraction shows they embed no rule strings and carry neither retracted claim, so there was nothing in them for the rewrite to make stale. The superseded Astro and npm bytes are kept under `dist/superseded/public-0.1.0-pre-rule-rewrite/`. The repack retires the gate evidence in the columns to the left for the Astro and npm rows; nothing here renews it. Five of the seven npm packages are `0.0.0-private` with `"private": true`; publishing means removing that flag deliberately. The website consumes these as vendored tarballs from `vendor/content-integrity/`, so publishing changes how the site installs them and needs coordinating. Owner-interactive npm 0.1.0 publication with account 2FA and no provenance; later npm OIDC/provenance; PyPI trusted publishing after npm live verification |
...
| Chrome extension 1.0.0 | exact local Web Store ZIP and listing bundle, rebuilt 30 August 2026 after the rule-string rewrite | **open** — the independent automated package/submission audit was passed by the superseded ZIP `061f5306...a1a93` | exact 15-file ZIP `e0fb2a0a460223db524c317d680bb71d39076af9c8b3362f96751e8ba1740ec8`, superseding `27272820c8…` which carried the withdrawn length figures and the retracted carrier claim; the submission validator passes on it and was shown to fail beforehand; **open** — unit/type 5/5, audit 0, Chrome 151/145 40/40 per lane and the seven-state axe/Worker parity belong to the superseded ZIP and must be re-run | no | no | no | Owner Chrome Stable Load unpacked/native actions and VoiceOver; live URLs; account/trader/regions/submission choices and public verification |
