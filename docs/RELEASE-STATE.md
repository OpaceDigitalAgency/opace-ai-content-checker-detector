...
WordPress 1.1.11 was built byte-for-byte identically on macOS and Linux with SHA-256 `b3ff4cffcabfdc2fcbe52604b02e28852505c01df3e8d66e81ecb505b4610dbd`; local-only test screenshots are explicitly excluded. Chrome 1.2.3 was rebuilt after the shared writing-rule correction with identical SHA-256 `f1aa497a009cff8073af3f218be8f0d045c63fd01515fb7b476968a283a4ec74`. These are test candidates, not published releases. Exact package paths are in the [submission manifest](../submission-prep/submission-manifest.json).

Chrome's pinned-icon page-capture failure was reproduced and fixed without adding broad permissions. After reloading the unpacked build, the owner confirmed page capture and the section-to-page jump work in their regular Chrome. The long-result horizontal overflow is removed. Export now has an immediately visible Back to report control, and every available step in the top rail is keyboard-operable. The full 67-state browser journey passed at 900 and 375 CSS pixels with zero overflow, axe violations or console errors. WordPress editor-to-report handover uses single-use browser session storage rather than uploading the draft to a server transient.
