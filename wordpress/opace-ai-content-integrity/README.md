...
Opace AI Content Integrity gives WordPress editors explicit local, on-device and optional EU-server analysis routes. It protects facts and citations, supports Block and Classic Editor working copies, and can save a hash-only evidence receipt without storing the draft text.

Version 1.0.10 is an in-development WordPress parity update, not a release candidate. It requires WordPress 6.5 or newer and PHP 7.4 or newer. Generated rewrites, commercial detector calls and an official Anthropic watermark verifier are not included; unavailable methods remain explicitly labelled rather than inferred.
...
> Release state: 1.0.10 remains a development line. It has not been installed into the shared local WordPress sites, submitted to WordPress.org or accepted by the owner; installed-byte, current/minimum WordPress, Plugin Check, owner Safari and VoiceOver acceptance remain separate gates.
...
1. Open **Content Integrity > Suite** and choose on-device Cycle-5, the available EU route, or integrity checks only.
