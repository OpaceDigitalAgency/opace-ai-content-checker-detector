...
export function adaptLegacyAnalysisResult(result, options) {
  ...
  const checkerResult = {
    ...
    exports: {
      ...
      report: { available: false, format: 'none', contains_content: false, explicit_user_action: true, complete_evidence: false, product_identity: 'Opace AI Content Integrity', support_destination: 'https://opace.agency/tools/ai/content-verification-integrity/' },
    },
    abuse_controls: {
      max_words: null,
...
export function renderResultShell(presentation) {
  ...
  return `<article class="oaci-result-shell" data-oaci-result-shell data-result-status="${escapeResultHtml(ai.assessment_status)}" data-result-profile="${escapeResultHtml(result.profile)}">
    <header class="oaci-result-mast"><div class="oaci-result-brand"><span class="oaci-result-mark">${mark}</span><div><p class="oaci-result-kicker">Opace AI Content Integrity</p><h2>Evidence report</h2><p>${escapeResultHtml(result.source.word_count.toLocaleString('en-GB'))} words · ${escapeResultHtml(presentation.surface)}</p></div></div><div class="oaci-result-overall"><span>${assessed ? 'Model score' : 'Model status'}</span><strong>${escapeResultHtml(overall)}</strong><b>${escapeResultHtml(assessed ? CHECKER_LEVEL_LABELS[ai.level] : 'Not assessed')}</b></div></header>
    ${renderGauge(result)}
