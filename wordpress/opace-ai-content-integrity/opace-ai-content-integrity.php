 ...
 * Text Domain: opace-ai-content-integrity
 *
 * @package OpaceAIContentIntegrity
 */

defined( 'ABSPATH' ) || exit;

define( 'OPACE_CONTENT_INTEGRITY_VERSION', '1.0.15' );
	...
	static function () {
		if ( version_compare( PHP_VERSION, '7.4', '<' ) || version_compare( get_bloginfo( 'version' ), '6.5', '<' ) ) {
			add_action(
				'admin_notices',
				static function () {
					echo '<div class="notice notice-error"><p>' . esc_html__( 'Opace AI Content Integrity requires WordPress 6.5 and PHP 7.4 or newer.', 'opace-ai-content-integrity' ) . '</p></div>';
				}
			);
			return;
		}
