...
test('version identity is aligned before package build', async () => {
	...
	assert.match(bootstrap, /\* Version: 1\.0\.8/);
	assert.match(bootstrap, /OPACE_CONTENT_INTEGRITY_VERSION', '1\.0\.8'/);
	assert.match(readme, /Stable tag: 1\.0\.8/);
	assert.match(citation, /^version: 1\.0\.8$/m);
