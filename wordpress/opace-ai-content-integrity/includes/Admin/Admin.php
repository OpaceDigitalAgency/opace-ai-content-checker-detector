...
final class Admin {
	...
	public function menu() {
		add_menu_page( __( 'Opace AI Content Integrity', 'opace-ai-content-integrity' ), __( 'AI Content Integrity', 'opace-ai-content-integrity' ), 'edit_posts', 'oaci-lab', array( $this, 'lab' ), 'dashicons-shield-alt', 58 );
		add_submenu_page( 'oaci-lab', __( 'Opace AI Content Integrity', 'opace-ai-content-integrity' ), __( 'Checker', 'opace-ai-content-integrity' ), 'edit_posts', 'oaci-lab', array( $this, 'lab' ) );
		add_submenu_page( 'oaci-lab', __( 'Receipts · Opace AI Content Integrity', 'opace-ai-content-integrity' ), __( 'Receipts', 'opace-ai-content-integrity' ), 'edit_posts', 'oaci-receipts', array( $this, 'receipts' ) );
		add_submenu_page( 'oaci-lab', __( 'Settings · Opace AI Content Integrity', 'opace-ai-content-integrity' ), __( 'Settings', 'opace-ai-content-integrity' ), 'manage_options', 'oaci-settings', array( $this, 'settings_page' ) );
		add_submenu_page( 'oaci-lab', __( 'Methods & privacy · Opace AI Content Integrity', 'opace-ai-content-integrity' ), __( 'Methods & privacy', 'opace-ai-content-integrity' ), 'manage_options', 'oaci-methods', array( $this, 'methods' ) );
	}
	...
	/**
	 * Adds "Check with AI Content Integrity" to the Posts and Pages list tables.
	 *
	...
	public function row_action( $actions, $post ) {
		...
		$actions['oaci_check'] = '<a href="' . esc_url( $url ) . '">' . esc_html__( 'Check with AI Content Integrity', 'opace-ai-content-integrity' ) . '</a>';
		return $actions;
	}
	...
	public function onboarding() {
		...
		echo '<div class="notice notice-info is-dismissible"><p><strong>' . esc_html__( 'Opace AI Content Integrity is ready.', 'opace-ai-content-integrity' ) . '</strong> ' . esc_html__( 'Add a draft, choose how it runs, then read what the checks found.', 'opace-ai-content-integrity' ) . ' <a href="' . esc_url( $url ) . '">' . esc_html__( 'Open the checker', 'opace-ai-content-integrity' ) . '</a></p></div>';
	}
	...
	public function receipts() {
		...
		$this->header( __( 'Receipts', 'opace-ai-content-integrity' ), __( 'Hash-only evidence this site kept. A receipt holds hashes and check results, never your text.', 'opace-ai-content-integrity' ), 'oaci-receipts' );
		...
		if ( empty( $rows ) ) {
			...
			echo '<h3>' . esc_html__( 'No receipts yet', 'opace-ai-content-integrity' ) . '</h3>';
			echo '<p>' . esc_html__( 'Run a check, then choose “Save hash-only receipt” to keep evidence of it here.', 'opace-ai-content-integrity' ) . '</p>';
			echo '<a class="oaci-button" href="' . esc_url( admin_url( 'admin.php?page=oaci-lab' ) ) . '">' . esc_html__( 'Open the checker', 'opace-ai-content-integrity' ) . '</a></div></div></div>';
			return;
		}
		echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '">';
		echo '<input type="hidden" name="action" value="oaci_delete_receipts">';
		wp_nonce_field( 'oaci_delete_receipts' );
		echo '<div class="oaci-table-wrap"><table class="oaci-table"><thead><tr><th class="check-column"><span class="screen-reader-text">' . esc_html__( 'Select', 'opace-ai-content-integrity' ) . '</span></th><th>' . esc_html__( 'Date', 'opace-ai-content-integrity' ) . '</th><th>' . esc_html__( 'Where it ran', 'opace-ai-content-integrity' ) . '</th><th>' . esc_html__( 'Receipt ID', 'opace-ai-content-integrity' ) . '</th><th>' . esc_html__( 'Hash', 'opace-ai-content-integrity' ) . '</th></tr></thead><tbody>';
		foreach ( $rows as $row ) {
			echo '<tr><th class="check-column"><input type="checkbox" name="receipt_ids[]" value="' . esc_attr( $row['public_id'] ) . '" aria-label="' . esc_attr( sprintf( /* translators: %s: the receipt identifier. */ __( 'Select receipt %s', 'opace-ai-content-integrity' ), $row['public_id'] ) ) . '"></th><td data-label="' . esc_attr__( 'Date', 'opace-ai-content-integrity' ) . '">' . esc_html( $row['created_at'] ) . '</td><td data-label="' . esc_attr__( 'Where it ran', 'opace-ai-content-integrity' ) . '">' . esc_html( $row['caller'] ) . '</td><td data-label="' . esc_attr__( 'Receipt ID', 'opace-ai-content-integrity' ) . '"><code>' . esc_html( $row['public_id'] ) . '</code></td><td data-label="' . esc_attr__( 'Hash', 'opace-ai-content-integrity' ) . '"><code>' . esc_html( $row['receipt_hash'] ) . '</code></td></tr>';
		}
		echo '</tbody></table></div><p><button type="submit" class="oaci-button" name="bulk_action" value="delete">' . esc_html__( 'Delete selected receipts', 'opace-ai-content-integrity' ) . '</button></p></form></div></div>';
	}
	...
	public function settings_page() {
		...
		$this->header( __( 'Settings', 'opace-ai-content-integrity' ), __( 'Where checks run, how much text they accept, and what happens when you remove the plugin.', 'opace-ai-content-integrity' ), 'oaci-settings' );
		...
		echo '<p class="oaci-group__help">' . esc_html__( 'The full checker always lives on its own screen. These add a smaller quick check beside the post.', 'opace-ai-content-integrity' ) . '</p>';
		$this->switch_row( 'editor_sidebar', __( 'Quick check in the block editor', 'opace-ai-content-integrity' ), __( 'A panel in the sidebar, beside the post you are writing.', 'opace-ai-content-integrity' ), $value );
		$this->switch_row( 'classic_meta_box', __( 'Quick check in the Classic Editor', 'opace-ai-content-integrity' ), __( 'A box in the right-hand column. It never appears in the block editor.', 'opace-ai-content-integrity' ), $value );
		echo '</section>';

		echo '<section class="oaci-group">';
		echo '<h2>' . esc_html__( 'How much text this site accepts', 'opace-ai-content-integrity' ) . '</h2>';
		echo '<p class="oaci-group__help">' . esc_html__( 'A longer draft is refused with a clear message. It is never quietly cut short and checked anyway.', 'opace-ai-content-integrity' ) . '</p>';
		echo '<div class="oaci-field"><label for="oaci-max-chars">' . esc_html__( 'Longest draft, in characters', 'opace-ai-content-integrity' ) . '</label>';
		...
		echo '<p class="oaci-field__help" id="oaci-max-chars-help">' . esc_html__( 'Between 10,000 and 100,000.', 'opace-ai-content-integrity' ) . '</p></div>';
		echo '<div class="oaci-stats">';
		$this->stat( number_format_i18n( (int) $value['max_chars'] ), __( 'characters in one run', 'opace-ai-content-integrity' ) );
		$this->stat( number_format_i18n( self::MODEL_MIN_WORDS ), __( 'words needed for an AI reading', 'opace-ai-content-integrity' ) );
		$this->stat( '20 MB', __( 'largest file for a Content Credentials check', 'opace-ai-content-integrity' ) );
		echo '</div>';
		$this->disclosure( __( 'Every limit, written out', 'opace-ai-content-integrity' ), $this->bullets( $this->limits_sentences() ) );
		echo '</section>';

		echo '<section class="oaci-group" aria-labelledby="oaci-server-settings">';
		echo '<h2 id="oaci-server-settings">' . esc_html__( 'Private EU analysis', 'opace-ai-content-integrity' ) . '</h2>';
		echo '<p class="oaci-group__help">' . esc_html__( 'The route that sends a draft to our EU server instead of running the model in the browser. It is off until you turn it on.', 'opace-ai-content-integrity' ) . '</p>';
		$this->switch_row( 'server_analysis_opt_in', __( 'Let editors choose private EU analysis', 'opace-ai-content-integrity' ), __( 'They still have to press a button that names the transfer for each run.', 'opace-ai-content-integrity' ), $value );
		echo '<p class="oaci-group__help"><strong>' . esc_html__( 'Right now:', 'opace-ai-content-integrity' ) . '</strong> ' . esc_html( $this->server_state_label( $server['state'] ) ) . '</p>';
		echo '<div class="oaci-stats">';
		$this->stat( number_format_i18n( ServerRateLimiter::MINUTE_LIMIT ), __( 'EU runs a minute, for each person', 'opace-ai-content-integrity' ) );
		$this->stat( number_format_i18n( ServerRateLimiter::HOUR_LIMIT ), __( 'EU runs an hour, for each person', 'opace-ai-content-integrity' ) );
		foreach ( $this->service_figures() as $figure ) {
			$this->stat( $figure['value'], $figure['label'] );
		}
		$this->stat( __( 'No limit', 'opace-ai-content-integrity' ), __( 'runs on the editor’s own device', 'opace-ai-content-integrity' ) );
		echo '</div>';
		echo '<p class="oaci-field__help">' . esc_html__( 'Figures above come from the service’s own status reply and are only as fresh as the last few minutes. Where it publishes no figure, none is shown.', 'opace-ai-content-integrity' ) . '</p>';
		$this->disclosure(
			__( 'What turning this on does', 'opace-ai-content-integrity' ),
			$this->para( __( 'Once it is on and the service is reachable, private EU analysis becomes the route the checker offers first, because it needs no download and gives an answer in about a second. An editor still presses a button that names the one-off transfer for that run. The draft goes from their browser to this WordPress site, and once from here to a fixed Opace endpoint. The plugin does not keep that draft, and never puts it in a link, a receipt or a log.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'Running on the editor’s own device stays available whether this is on or off, and the checker offers it in one click whenever the EU route is unavailable, refuses a run or fails.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'You cannot change the destination. It is fixed in the plugin code so that a compromised administrator account, or a copied configuration file, cannot quietly point drafts somewhere else.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'What the draft goes through: it is read once in memory in europe-west1 to produce the reading, and is not kept afterwards. The service reports that it retains nothing, and the plugin refuses any answer that does not.', 'opace-ai-content-integrity' ) )
		);
		$this->disclosure( __( 'How the shared allowance is divided', 'opace-ai-content-integrity' ), $this->bullets( $this->allowance_sentences() ) );
		echo '</section>';

		echo '<section class="oaci-group">';
		echo '<h2>' . esc_html__( 'The on-device model download', 'opace-ai-content-integrity' ) . '</h2>';
		echo '<p class="oaci-group__help">' . esc_html__( 'Nothing to set here. This is what an editor is asked to download the first time they run on their own device.', 'opace-ai-content-integrity' ) . '</p>';
		echo '<div class="oaci-stats">';
		$this->stat( self::MODEL_DOWNLOAD_LABEL, __( 'of model weights, once, then cached', 'opace-ai-content-integrity' ) );
		$this->stat( 'SHA-256', __( 'checked before anything reads the file', 'opace-ai-content-integrity' ) );
		$this->stat( __( 'One click', 'opace-ai-content-integrity' ), __( 'to remove it again from the checker', 'opace-ai-content-integrity' ) );
		echo '</div>';
		$this->disclosure(
			__( 'What editors are asked to download', 'opace-ai-content-integrity' ),
			$this->para( $this->model_download_sentence() )
			. $this->para( __( 'The program that reads the file is the inference engine bundled inside this plugin and served from your own site. No executable code is fetched from anywhere else, and the draft is never sent with the download request.', 'opace-ai-content-integrity' ) )
		);
		echo '</section>';

		echo '<section class="oaci-group">';
		echo '<h2>' . esc_html__( 'When you remove the plugin', 'opace-ai-content-integrity' ) . '</h2>';
		echo '<p class="oaci-group__help">' . esc_html__( 'Either way, receipts hold hashes and check results, never your text, and the plugin keeps no event log.', 'opace-ai-content-integrity' ) . '</p>';
		$this->switch_row( 'delete_data_uninstall', __( 'Delete this plugin’s data when it is uninstalled', 'opace-ai-content-integrity' ), __( 'Leave this off to keep saved receipts if you remove the plugin.', 'opace-ai-content-integrity' ), $value );
		echo '</section>';

		echo '<p class="oaci-submit"><button type="submit" class="oaci-button oaci-button--primary">' . esc_html__( 'Save settings', 'opace-ai-content-integrity' ) . '</button></p>';
		echo '</form></div>';
	}
	...
	private function service_figures() {
		...
		$tiles   = array();
		$map     = array(
			'channel_floor'         => __( 'section readings a day kept for WordPress sites', 'opace-ai-content-integrity' ),
			'channel_remaining'     => __( 'left in that share, when this page last asked', 'opace-ai-content-integrity' ),
			'shared_pool_remaining' => __( 'left in the pool every surface shares', 'opace-ai-content-integrity' ),
			'site_per_hour'         => __( 'section readings an hour for this whole site', 'opace-ai-content-integrity' ),
			'site_per_day'          => __( 'section readings a day for this whole site', 'opace-ai-content-integrity' ),
		);
	...
	public function methods() {
		$server = $this->server_analysis->status();
		$this->header( __( 'Methods & privacy', 'opace-ai-content-integrity' ), __( 'What runs, where it runs, and what a result can and cannot tell you.', 'opace-ai-content-integrity' ), 'oaci-methods' );
		echo '<div class="oaci-panel">';
		echo '<section class="oaci-group">';
		echo '<h2>' . esc_html__( 'The short version', 'opace-ai-content-integrity' ) . '</h2>';
		echo '<p class="oaci-group__help">' . esc_html__( 'No checker can prove who wrote a piece of text. This one reads patterns and shows you the evidence, in your own sentences, so you can judge for yourself. A finding is somewhere to look, not a verdict about a person.', 'opace-ai-content-integrity' ) . '</p>';
		echo '<div class="oaci-methods-grid">';

		$this->method_card(
			__( 'Runs in your browser, always', 'opace-ai-content-integrity' ),
			array(
				__( 'Hidden characters, lookalike letters, writing patterns and Content Credentials in files.', 'opace-ai-content-integrity' ),
				__( 'These four checks need no download and send nothing anywhere.', 'opace-ai-content-integrity' ),
			),
			$this->bullets(
				array(
					__( 'Hidden and invisible characters, and mixed-script lookalike letters (unicode:2026.08.2)', 'opace-ai-content-integrity' ),
					__( 'Writing patterns and editing suggestions (en-signals:2026.08.6)', 'opace-ai-content-integrity' ),
					__( 'Content Credentials in JPEG, PNG, WebP and PDF files (c2pa-web:0.14.3)', 'opace-ai-content-integrity' ),
					__( 'Protected numbers, dates, links, quotations, citations and code', 'opace-ai-content-integrity' ),
				)
			) . $this->para( __( 'The Content Credentials check never fetches a remote manifest, a certificate status or a trust list, so it will not tell you a signer is trusted. Present, absent, invalid and untrusted stay separate answers.', 'opace-ai-content-integrity' ) )
		);

		$this->method_card(
			__( 'On this device', 'opace-ai-content-integrity' ),
			array(
				__( 'The trained model runs in the editor’s browser, so the draft is not sent to Opace or to this site for scoring.', 'opace-ai-content-integrity' ),
				__( 'The first run downloads the model once; after that it is cached, and the route has no run limit.', 'opace-ai-content-integrity' ),
			),
			$this->para( $this->model_download_sentence() )
			. $this->para( __( 'On this route the run has no limit because it is the editor’s own computer doing the work. Saving a receipt afterwards sends its hashes, never the text, to this site.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'The program that reads the file is the inference engine bundled inside this plugin, which WordPress serves from your own site. Nothing executable is fetched from anywhere else.', 'opace-ai-content-integrity' ) ),
			__( 'Private, no limit', 'opace-ai-content-integrity' ),
			'pass'
		);

		$this->method_card(
			__( 'Private EU analysis', 'opace-ai-content-integrity' ),
			array(
				__( 'The draft goes once through this site to our EU service, is read there in memory, and is not kept.', 'opace-ai-content-integrity' ),
				__( 'It needs three separate yeses, and it has allowances that running on the device does not.', 'opace-ai-content-integrity' ),
			),
			$this->para( $this->server_state_label( $server['state'] ) )
			. $this->para( __( 'The three yeses: an administrator turns the route on, the site confirms the service can be reached, and the editor presses a button that names the one-off transfer. Only then does the draft travel. The plugin never pretends to be a browser by faking an Origin or a user agent.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'When all three are true this is the route the checker offers first, because there is nothing to download and the answer comes back in about a second. The draft is read once in memory in europe-west1 and is not retained; the plugin refuses any answer that does not say so.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'Whenever this route is unavailable, refuses a run or fails, the checker says which allowance was reached and when it comes back, and offers to run the same model on the device in one click.', 'opace-ai-content-integrity' ) )
			. $this->bullets( $this->allowance_sentences() ),
			'ready' === $server['state'] ? __( 'Available', 'opace-ai-content-integrity' ) : __( 'Not available', 'opace-ai-content-integrity' ),
			'ready' === $server['state'] ? 'pass' : 'not_run'
		);

		$this->method_card(
			__( 'Not available in this release', 'opace-ai-content-integrity' ),
			array(
				__( 'Three things other tools claim, which this one will not.', 'opace-ai-content-integrity' ),
				__( 'Each reports its own state and stops, rather than offering a stand-in.', 'opace-ai-content-integrity' ),
			),
			$this->para( __( 'Anthropic official watermark verifier: Unsupported. There is no official detector we can call, so this check reports Unsupported and stops. A public watermark test or a writing-pattern result is not a stand-in for it, and we will not present one as though it were.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'Claude readiness: not supported. There is no readiness check in this release, so the plugin does not offer one.', 'opace-ai-content-integrity' ) )
			. $this->para( __( 'Rewrite Lab: not configured. Generated rewrites need a text-generation service, which this release does not include. Safe character fixes in the checker are a separate thing: they change characters and spacing only, and you preview them first.', 'opace-ai-content-integrity' ) ),
			__( 'Unsupported', 'opace-ai-content-integrity' ),
			'unsupported'
		);

		$this->method_card(
			__( 'Where your text goes', 'opace-ai-content-integrity' ),
			array(
				__( 'One line per route, because a single claim about the whole plugin could not stay true.', 'opace-ai-content-integrity' ),
				__( 'Shared summaries, downloaded JSON and links never carry your text.', 'opace-ai-content-integrity' ),
			),
			$this->bullets(
				array(
					__( 'Character, writing and file checks: your browser only.', 'opace-ai-content-integrity' ),
					__( 'On-device analysis: your browser only. Model files come down; the draft does not go up.', 'opace-ai-content-integrity' ),
					__( 'Saving a receipt: the draft is sent to this WordPress site so it can be hashed, and only hashes and check results are stored.', 'opace-ai-content-integrity' ),
					__( 'Private EU analysis: the draft is sent once, and only when an administrator has turned the route on and you have pressed the button that names the transfer. It is read in memory in europe-west1 and is not retained afterwards.', 'opace-ai-content-integrity' ),
					__( 'Shared summaries, downloaded JSON and links never carry your text or the passages we quote back to you.', 'opace-ai-content-integrity' ),
				)
			)
		);

		$this->method_card(
			__( 'How much it will check at once', 'opace-ai-content-integrity' ),
			array(
				sprintf(
					/* translators: 1: character limit, 2: minimum word count. */
					__( 'Up to %1$s characters in one run, and at least %2$s words for an AI reading.', 'opace-ai-content-integrity' ),
					number_format_i18n( $this->limits()['max_chars'] ),
					number_format_i18n( self::MODEL_MIN_WORDS )
				),
				__( 'Editors see these in the checker as plain messages, never as an error code.', 'opace-ai-content-integrity' ),
			),
	...
	private function method_card( $title, array $lines, $detail_html, $chip = '', $chip_state = 'pass' ) {
		...
		$this->disclosure( __( 'Details', 'opace-ai-content-integrity' ), $detail_html );
		echo '</div>';
	}
	...
	private function limits_sentences() {
		...
		return array(
			...
			__( 'On this device: no run limit. It is the editor’s own computer doing the work, so it is the one route that cannot run out.', 'opace-ai-content-integrity' ),
		);
	}
	...
	private function suite_nav( $current ) {
		$items = array(
			'oaci-lab'      => __( 'Checker', 'opace-ai-content-integrity' ),
			'oaci-receipts' => __( 'Receipts', 'opace-ai-content-integrity' ),
		);
		if ( current_user_can( 'manage_options' ) ) {
			$items['oaci-methods']  = __( 'Methods & privacy', 'opace-ai-content-integrity' );
			$items['oaci-settings'] = __( 'Settings', 'opace-ai-content-integrity' );
		}
		echo '<nav class="oaci-suite-nav" aria-label="' . esc_attr__( 'AI Content Integrity', 'opace-ai-content-integrity' ) . '">';
	...
	private function server_state_label( $state ) {
		$labels = array(
			...
			'endpoint_missing'    => __( 'On here, but this build carries no endpoint, so the route cannot be offered. Editors get the on-device route instead.', 'opace-ai-content-integrity' ),
			'channel_unavailable' => __( 'On here, but the service is not accepting WordPress runs at the moment. Editors get the on-device route instead.', 'opace-ai-content-integrity' ),
			'checking'            => __( 'On here, and nobody has asked the service yet. The checker asks as it opens and says what came back.', 'opace-ai-content-integrity' ),
			'ready'               => __( 'On and reachable, so the checker offers it first. Running on the editor’s own device stays one click away.', 'opace-ai-content-integrity' ),
		);
