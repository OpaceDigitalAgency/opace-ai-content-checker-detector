	...
	private function service_allowance_line() {
		...
		if ( null !== $per_hour && null !== $per_day ) {
			return sprintf(
				...
				__( 'The service holds this whole site to %1$s section readings an hour and %2$s a day. A section is roughly four hundred words.', 'opace-ai-content-integrity' ),
				number_format_i18n( $per_hour ),
				number_format_i18n( $per_day )
			);
		}
		return __( 'The service holds this whole site to its own hourly and daily ceiling. If a run is refused we name which allowance it was and when it comes back.', 'opace-ai-content-integrity' );
	}
		...
		<div class="wrap oaci-wrap">
			<div class="oaci-header">
				...
				<div>
					...
					<p><?php esc_html_e( 'Read a draft the way our free online checker does. Add the text, pick where it runs, then see what the model found and why.', 'opace-ai-content-integrity' ); ?></p>
				</div>
			</div>
			<nav class="oaci-suite-nav" aria-label="<?php esc_attr_e( 'AI Content Integrity', 'opace-ai-content-integrity' ); ?>">
			...
			<div class="oaci-lab" id="oaci-lab-root" data-oaci-lab>
				...
				<section class="oaci-panel" id="oaci-step-draft">
					<div class="oaci-step-head" id="checker">
						...
						<div>
							<h2><?php esc_html_e( 'Add your draft', 'opace-ai-content-integrity' ); ?></h2>
							<p><?php esc_html_e( 'Paste it in, open a file, or try one of our examples.', 'opace-ai-content-integrity' ); ?></p>
						</div>
					</div>
					...
					<div class="oaci-input-panel" id="oaci-panel-paste" role="tabpanel" aria-labelledby="oaci-tab-paste" tabindex="0">
						...
						<textarea id="oaci-source" rows="10" maxlength="100000" placeholder="<?php esc_attr_e( 'Paste your draft here, or drop a text file onto this box…', 'opace-ai-content-integrity' ); ?>"></textarea>
					...
					<div class="oaci-input-panel" id="oaci-panel-upload" role="tabpanel" aria-labelledby="oaci-tab-upload" tabindex="0" hidden>
						<div class="oaci-dropzone" id="oaci-dropzone">
							...
							<p><?php esc_html_e( 'TXT, Markdown and HTML open in the box. JPEG, PNG, WebP and PDF run a Content Credentials check instead. Maximum file size: 20 MB.', 'opace-ai-content-integrity' ); ?></p>
							<label class="oaci-button" for="oaci-source-file"><?php esc_html_e( 'Choose a file', 'opace-ai-content-integrity' ); ?></label>
						...
						<details class="oaci-disclosure">
							<summary><?php esc_html_e( 'What happens to a file you open', 'opace-ai-content-integrity' ); ?></summary>
							<div class="oaci-disclosure__body">
								<p><?php esc_html_e( 'The file you choose is read in this browser and is not sent to Opace or to this site. Remote manifests, certificate status and trust lists are not fetched, so we never claim a signer is trusted.', 'opace-ai-content-integrity' ); ?></p>
							</div>
						</details>
					</div>
				...
				<section class="oaci-panel" id="oaci-step-route">
					<div class="oaci-step-head">
						...
						<div>
							<h2><?php esc_html_e( 'Choose how it runs', 'opace-ai-content-integrity' ); ?></h2>
							<p><?php esc_html_e( 'One is always selected. The button below says exactly what pressing it will do.', 'opace-ai-content-integrity' ); ?></p>
						</div>
					</div>
					<fieldset class="oaci-routes">
						...
						<label class="oaci-route<?php echo $server_available ? '' : ( $server_checking ? ' is-checking' : ' is-unavailable' ); ?>" data-oaci-route-card="server">
							...
							<span>
								...
								<?php
								if ( $server_available ) {
									esc_html_e( 'Nothing to download. Your draft goes once to our EU server, which reads it and does not keep it.', 'opace-ai-content-integrity' );
								} elseif ( $server_checking ) {
									esc_html_e( 'We are asking the EU service whether it is accepting runs. On this device is ready now.', 'opace-ai-content-integrity' );
								} else {
									esc_html_e( 'An administrator has not turned it on or the service is not accepting runs. Use on this device instead.', 'opace-ai-content-integrity' );
								}
								?>
								</small>
							</span>
						...
						<label class="oaci-route" data-oaci-route-card="on_device">
							...
							<span>
								...
								<small><?php esc_html_e( 'The model runs in your browser. On this route the draft is not sent to Opace or to this site.', 'opace-ai-content-integrity' ); ?></small>
							</span>
						</label>
						<label class="oaci-route" data-oaci-route-card="local">
							<input type="radio" name="oaci-analysis-route" value="local">
							<span>
								<span class="oaci-route-tag oaci-route-tag--alternative" data-oaci-route-tag="local"><?php esc_html_e( 'No AI reading', 'opace-ai-content-integrity' ); ?></span>
								<strong><?php esc_html_e( 'Integrity checks only', 'opace-ai-content-integrity' ); ?></strong>
								<small><?php esc_html_e( 'Hidden characters, lookalike letters and writing patterns. This route does not produce an AI-pattern score.', 'opace-ai-content-integrity' ); ?></small>
							</span>
						</label>
					</fieldset>
					...
					<div class="oaci-run">
						<div class="oaci-run__action">
							<p class="oaci-run__note" id="oaci-primary-note"><?php esc_html_e( 'Add at least 60 words for an AI reading. Shorter drafts still get the character and writing checks.', 'opace-ai-content-integrity' ); ?></p>
							<button type="button" class="oaci-button oaci-button--primary" id="oaci-inspect"><?php esc_html_e( 'Check my draft', 'opace-ai-content-integrity' ); ?></button>
						</div>
						<div class="oaci-model-line" id="oaci-model-download" hidden>
							<span class="oaci-model-line__state" id="oaci-model-cache-state"><?php esc_html_e( 'We will look for a model already on this device when you press the button. That check needs no network.', 'opace-ai-content-integrity' ); ?></span>
							<button type="button" class="oaci-button oaci-button--quiet" id="oaci-clear-model-cache"><?php esc_html_e( 'Clear downloaded model', 'opace-ai-content-integrity' ); ?></button>
						</div>
						<details class="oaci-disclosure" id="oaci-route-disclosure">
							<summary><?php esc_html_e( 'How this route works', 'opace-ai-content-integrity' ); ?></summary>
							<div class="oaci-disclosure__body">
								<p id="oaci-route-detail"><?php echo $server_available ? esc_html__( 'When you press the button, the draft goes once through this WordPress site to our fixed EU service, is read there in memory to produce the reading, and is not kept. If that route is busy or has reached an allowance we say so and offer to run it on this device instead.', 'opace-ai-content-integrity' ) : esc_html__( 'The model runs in this browser. On this route your draft is not sent to Opace or to this site for scoring; only the pinned model files download, and only when you press a button that says so.', 'opace-ai-content-integrity' ); ?></p>
								...
								printf(
									...
									esc_html__( 'The download is %1$s of model weights: %2$s, %3$s bytes, SHA-256 begins %4$s. It is a data file, not a program. Your browser checks it against a hash published in this plugin before anything reads it, keeps it cached like any other web asset, and you can remove it with one click.', 'opace-ai-content-integrity' ),
								...
								<h3 class="oaci-eyebrow"><?php esc_html_e( 'How much this site will check at once', 'opace-ai-content-integrity' ); ?></h3>
								<ul class="oaci-usage-limits">
									...
									printf(
										...
										esc_html__( 'Private EU analysis: %1$s runs a minute and %2$s an hour for each person, so one account cannot use up this site’s share.', 'opace-ai-content-integrity' ),
									...
									<li><?php esc_html_e( 'On this device: no run limit at all. It is your own computer doing the work, so it is the one route that cannot run out.', 'opace-ai-content-integrity' ); ?></li>
								</ul>
							</div>
						</details>
						<div id="oaci-run-progress" class="oaci-progress" role="status" aria-live="polite" hidden>
							...
							<button type="button" class="oaci-button" id="oaci-cancel-run"><?php esc_html_e( 'Cancel this run', 'opace-ai-content-integrity' ); ?></button>
						</div>
					</div>
				</section>
				...
				<section class="oaci-panel oaci-result-panel" id="oaci-result-panel" aria-labelledby="oaci-evidence-title" tabindex="0">
					<div class="oaci-result-head">
						<div>
							...
							<p><?php esc_html_e( 'Evidence to read, not proof of who wrote it.', 'opace-ai-content-integrity' ); ?></p>
						</div>
					</div>
					<div class="oaci-toolbar" id="oaci-toolbar" hidden>
						<button type="button" class="oaci-button" id="oaci-print" disabled><?php $this->icon( 'print' ); ?><?php esc_html_e( 'Print', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-download-pdf" disabled><?php $this->icon( 'download' ); ?><?php esc_html_e( 'Download PDF', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-download-json" disabled><?php $this->icon( 'download' ); ?><?php esc_html_e( 'Download JSON receipt', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-copy-share" disabled><?php $this->icon( 'share' ); ?><?php esc_html_e( 'Copy share summary', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-save-receipt" disabled><?php $this->icon( 'receipt' ); ?><?php esc_html_e( 'Save hash-only receipt', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-preview-fixes" disabled><?php $this->icon( 'fix' ); ?><?php esc_html_e( 'Safe-fix preview', 'opace-ai-content-integrity' ); ?></button>
						<button type="button" class="oaci-button" id="oaci-show-protected" disabled><?php $this->icon( 'lock' ); ?><?php esc_html_e( 'Protected facts', 'opace-ai-content-integrity' ); ?></button>
					</div>
					...
					<div id="oaci-results">
						<div class="oaci-empty">
							...
							<p><?php esc_html_e( 'Add a draft, choose how it runs, then press the button in step two. Every check that ran, and every one that could not, stays on this page.', 'opace-ai-content-integrity' ); ?></p>
						</div>
					</div>
					<div id="oaci-fix-panel" class="oaci-local-card" tabindex="-1" hidden>
						<h3><?php esc_html_e( 'Safe-fix preview', 'opace-ai-content-integrity' ); ?></h3>
						<p class="oaci-local-card__lead"><?php esc_html_e( 'These fixes change characters and spacing only. They do not test for, or remove, a statistical watermark.', 'opace-ai-content-integrity' ); ?></p>
						<div id="oaci-fix-list"></div>
						<button type="button" class="oaci-button" id="oaci-apply-fixes"><?php esc_html_e( 'Apply selected fixes to the draft', 'opace-ai-content-integrity' ); ?></button>
					</div>
					<div id="oaci-protected" class="oaci-local-card" tabindex="-1" hidden>
						<h3><?php esc_html_e( 'Protected facts', 'opace-ai-content-integrity' ); ?></h3>
						<p class="oaci-local-card__lead"><?php esc_html_e( 'Numbers, dates, links, quotations, citations and code found in your draft. Safe fixes never touch these.', 'opace-ai-content-integrity' ); ?></p>
