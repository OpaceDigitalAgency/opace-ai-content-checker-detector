 ...
 */
import { documentCadence } from "./cadence.mjs";
...
const phrase = (text, aiPer1000, humanPer1000, aiDocs, humanDocs, ratio, confirmingRatio) => ({
 ...
 */
export const CURATED_PHRASE_TELLS = [
...
const phraseTokens = (text) => tokenise(text).map(token => token.text);
/** Curated phrases present in the draft, strongest ratio first. */
export const findPhraseTells = (draft) => {
    if (!draft.trim())
        return [];
...
const paragraphBlocks = (draft) => {
    ...
    for (const block of blocks) {
        ...
        if (!lines.length)
            continue;
        if (lines.length === 1 && headingLike(lines[0]))
            continue;
        paragraphs.push(lines.join("\n"));
    }
    return paragraphs;
};
const sentenceCount = (paragraph) => paragraph.split(/(?<=[.!?])\s+/).filter(part => part.trim().split(/\s+/).length >= 2).length;
 ...
 */
export const paragraphRhythm = (draft) => {
 ...
 */
export const formulaicCloser = (draft) => {
    const blocks = draft.split(/\n\s*\n/).map(block => block.trim()).filter(Boolean);
    if (!blocks.length)
        return undefined;
    ...
    for (const block of blocks) {
        ...
        for (const line of lines) {
            if ((headingLike(line) || MD_HEADING_RE.test(block)) && CLOSER_RE.test(line))
                return { line };
        }
    }
...
const whitespaceWords = (text) => text.split(/\s+/).filter(Boolean).length;
/** Words-per-paragraph regularity: population CV over paragraph blocks. */
export const paragraphSizeRegularity = (draft) => {
...
export const sectionLengthUniformity = (draft) => {
    if (whitespaceWords(draft) < REGULARITY_MIN_WORDS)
        return undefined;
...
export const COHESION_STOPWORDS = new Set(`a about above after again against all am an and any are aren't as at be because been
...
export const COHESION_WORD_RE = /[A-Za-zÀ-ɏ']+/g;
 ...
 * would suggest.
 */
export const cohesionSentences = (draft) => {
    const out = [];
 ...
 */
export const adjacentCohesionRaw = (draft) => {
...
export const adjacentCohesion = (draft) => {
...
const stripMdInline = (s) => s
    .replace(/\*\*(.+?)\*\*/g, "$1").replace(/\*(.+?)\*/g, "$1")
    .replace(/`(.+?)`/g, "$1").replace(/\[(.+?)\]\([^)]*\)/g, "$1").trim();
const classifyBlocks = (text) => {
    const out = [];
...
export const sectionScaffold = (draft) => {
    const blocks = classifyBlocks(draft);
    const sections = [];
    let current = [];
    for (const block of blocks) {
...
export const scaffoldFeaturesRaw = (text) => {
...
export const wordMetrics = (text) => {
    ...
    const out = {};
    if (nonempty.length >= 4)
        out.ppsVar = populationVariance(nonempty.map(section => section.length));
...
export const structureTextFromHtml = (html, plainText) => {
    ...
    // match, in order.
    const fingerprint = (text) => text.replace(/\s+/g, "");
    if (fingerprint(body.textContent ?? "") !== fingerprint(plainText))
        return undefined;
    const blocks = [];
    const emit = (line) => { const text = collapse(line); if (text)
        blocks.push(text); };
    const walk = (node) => {
 ...
 */
export const findDocumentTells = (draft, structureHtml) => {
