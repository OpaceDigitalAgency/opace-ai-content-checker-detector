...
export const cadenceWords = (s) => s.match(WORD_RE) ?? [];
export const splitCadenceSentences = (text) => {
    const out = [];
    for (const raw of text.trim().split(SENT_SPLIT)) {
        const piece = raw.trim();
        if (!piece)
            continue;
        if (out.length && ABBREV.test(out[out.length - 1]))
...
export const splitCadenceParagraphs = (text) => {
...
const startsImperative = (ws) => {
    ...
    if (IMPERATIVE_VERBS.has(first)) {
        if (ws.length > 1 && AUX_VERBS.has(ws[1].toLowerCase()))
            return false;
        return true;
    }
    return false;
};
const firstFiniteIndex = (ws) => {
    for (let i = 0; i < ws.length; i++) {
        const lw = ws[i].toLowerCase();
        if (AUX_VERBS.has(lw) || UTILITY_VERBS.has(lw))
            return i;
    }
    return -1;
};
const isAbstractSubject = (sub) => {
    if (!sub.length || sub.length > 5)
        return false;
    if (sub.some(w => FIRST_SECOND_PERSON.has(w.toLowerCase())))
        return false;
...
const countEnumeration = (s) => /\w+\s*,\s*[^,;.]{2,40},\s*[^,;.]{2,40}\s+(?:and|or)\s+/.test(s);
/** Exported mutable ONLY so the probe test can break it and prove the probe fails. */
export let isBalancedImpl = (s) => {
...
export const analyseCadenceSentence = (s) => {
    ...
    if (s.replace(/\s+$/, "").endsWith("?"))
        ...
        sent.role = "I";
    else if (CONSEQUENCE_OPENERS.some(o => low.startsWith(` ${o}`)))
        sent.role = "S";
    else if ([" should ", " must ", " need to ", " ought to "].some(m => low.includes(m)))
        sent.role = "R";
    else if (EXAMPLE_MARKERS.some(m => low.includes(m)))
        sent.role = "E";
    else if (CONTRAST_MARKERS.some(m => low.includes(m)))
        sent.role = "X";
    else if (sent.messy)
        sent.role = "A";
    else
...
const cadenceFrom = (ss) => {
    if (!ss.length)
        return 0;
    const total = ss.reduce((sum, s) => sum + s.nWords, 0);
    const roles = ss.map(s => s.role);
    let score = 0;
    if (ss.length >= 3 && total <= 55)
        score += 3;
    if (["I", "R", "S"].includes(roles[roles.length - 1]))
        score += 2;
    if (ss.length >= 3 && ["C", "X"].includes(roles[0]) && ["I", "R", "S"].includes(roles[roles.length - 1]))
        score += 2;
    if (ss.some(s => s.balanced))
        score += 2;
    score += Math.min(2, ss.filter(s => s.utility).length);
    if (ss.some(s => s.enumeration))
        score += 1;
    score -= 2 * ss.filter(s => s.messy).length;
    return Math.max(0, score);
};
/** Exported mutable ONLY so the probe test can break it and prove the probe fails. */
export let paragraphCadence = (paragraph) => cadenceFrom(splitCadenceSentences(paragraph).map(analyseCadenceSentence));
export const roleAnnotate = (text) => splitCadenceParagraphs(text).flatMap(p => splitCadenceSentences(p).map(s => {
...
export const __cadenceTestHooks = {
    breakImperatives() { const saved = new Set(IMPERATIVE_VERBS); IMPERATIVE_VERBS.clear(); return () => { for (const v of saved)
        IMPERATIVE_VERBS.add(v); }; },
    breakUtility() { const saved = new Set(UTILITY_VERBS); UTILITY_VERBS.clear(); return () => { for (const v of saved)
        UTILITY_VERBS.add(v); }; },
    ...
    constantScore(value) { const saved = paragraphCadence; paragraphCadence = () => value; return () => { paragraphCadence = saved; }; }
};
 ...
 */
export const documentCadence = (text) => {
 ...
 */
export const hasParagraphMarkup = (text) => splitCadenceParagraphs(text).length >= 3;
/**
 ...
 * gate).
 */
export const paragraphCadenceRate = (text) => {
    const paragraphs = splitCadenceParagraphs(text)
        .map(p => splitCadenceSentences(p).map(analyseCadenceSentence))
        .filter(ss => ss.length > 0);
    const sentences = paragraphs.flat();
    ...
    if (nWords < 50 || nSents < 4)
        return undefined;
    if (!hasParagraphMarkup(text))
        return undefined;
...
export const rolesInWords = (roles) => {
    const WORDS = {
