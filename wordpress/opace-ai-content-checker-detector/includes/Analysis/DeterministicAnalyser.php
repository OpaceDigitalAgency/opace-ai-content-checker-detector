...
final class DeterministicAnalyser {
	...
	public function analyse( array $request ) {
		if ( '1.0' !== ( isset( $request['schema_version'] ) ? $request['schema_version'] : '' ) || 0 !== strpos( (string) ( isset( $request['contract_version'] ) ? $request['contract_version'] : '' ), '1.' ) ) {
			return new WP_Error( 'contract_incompatible', __( 'The request contract is incompatible with this plugin.', 'opace-ai-content-checker-detector' ), array( 'status' => 400 ) );
		}
		...
		if ( ! in_array( 'wordpress_local', $allowed_routes, true ) ) {
			return new WP_Error( 'consent_required', __( 'This request does not permit the WordPress local inspection route.', 'opace-ai-content-checker-detector' ), array( 'status' => 409 ) );
		}
		...
		if ( '' === trim( $text ) ) {
			return new WP_Error( 'invalid_request', __( 'Add text to inspect.', 'opace-ai-content-checker-detector' ), array( 'status' => 400 ) );
		}
		$settings = Settings::get();
		if ( mb_strlen( $text, 'UTF-8' ) > $settings['max_chars'] ) {
			return new WP_Error( 'request_too_large', __( 'The text exceeds this site’s inspection limit.', 'opace-ai-content-checker-detector' ), array( 'status' => 413 ) );
		}
		if ( ! mb_check_encoding( $text, 'UTF-8' ) ) {
			return new WP_Error( 'invalid_request', __( 'The text is not valid UTF-8.', 'opace-ai-content-checker-detector' ), array( 'status' => 400 ) );
		}
