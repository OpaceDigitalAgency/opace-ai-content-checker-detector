	...
	public function analyse( $text, $request_id ) {
		...
		if ( ! $status['available'] ) {
			return new WP_Error(
				...
				__( 'The Opace EU server route is not available in this build.', 'opace-ai-content-checker-detector' ),
				array( 'status' => 503 )
			);
		}
		...
		if ( 0 !== strpos( $content_type, 'application/json' ) ) {
			return new WP_Error( 'invalid_server_response', __( 'The server returned a result this plugin cannot safely display.', 'opace-ai-content-checker-detector' ), array( 'status' => 502 ) );
		}
