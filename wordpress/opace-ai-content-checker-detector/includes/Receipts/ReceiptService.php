...
final class ReceiptService {
	...
	public function create_checker_result( array $result, $owner_id ) {
		...
		if ( is_wp_error( $valid ) || false !== strpos( wp_json_encode( $result ), '"passage"' ) || false !== strpos( wp_json_encode( $result ), '"content"' ) ) {
			return is_wp_error( $valid ) ? $valid : new \WP_Error( 'content_in_receipt', __( 'The full checker receipt must not contain draft text or passages.', 'opace-ai-content-checker-detector' ), array( 'status' => 400 ) );
		}
		$receipt = array(
			'schema_version'   => '1.0',
			'contract_version' => '1.0.0',
