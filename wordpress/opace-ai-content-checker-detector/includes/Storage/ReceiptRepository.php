...
final class ReceiptRepository {
	public function save( array $receipt, $job_id, $owner_id, $caller, $object_id ) {
		...
		if ( ! empty( $receipt['contains_content'] ) ) {
			return new WP_Error( 'consent_required', __( 'Content-bearing receipts are not enabled in this candidate.', 'opace-ai-content-checker-detector' ) );
		}
		...
		return false === $ok ? new WP_Error( 'receipt_save_failed', __( 'The hash-only receipt could not be saved.', 'opace-ai-content-checker-detector' ) ) : $receipt;
	}
	...
	public function get_for_job( $job_id, $owner_id ) {
		...
		return $json ? json_decode( $json, true ) : new WP_Error( 'object_not_found', __( 'The receipt was not found.', 'opace-ai-content-checker-detector' ), array( 'status' => 404 ) );
	}
