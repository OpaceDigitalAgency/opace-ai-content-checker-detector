...
final class JobRepository {
	public function find_idempotent( $owner_id, $idempotency_hash, $input_hash, $config_hash ) {
		...
		if ( ! hash_equals( $row['input_hash'], $input_hash ) || ! hash_equals( $row['config_hash'], $config_hash ) ) {
			return new WP_Error( 'idempotency_conflict', __( 'That idempotency key was already used for a different request.', 'opace-ai-content-checker-detector' ), array( 'status' => 409 ) );
		}
	...
	public function create( $owner_id, $caller, $object_id, $input_hash, $config_hash, $idempotency_hash = null ) {
		...
		if ( ( new Migrator() )->is_read_only() ) {
			return new WP_Error( 'legacy_storage_detected', __( 'Storage is read-only until the legacy data is reviewed.', 'opace-ai-content-checker-detector' ) );
		}
		$table     = $wpdb->prefix . 'opace_ci_jobs';
		...
		return false === $inserted ? new WP_Error( 'internal_error', __( 'The inspection session could not be created.', 'opace-ai-content-checker-detector' ) ) : $this->get( $public_id, $owner_id );
	}
	...
	public function get( $public_id, $owner_id ) {
		...
		return $row ? $this->normalise( $row ) : new WP_Error( 'object_not_found', __( 'The session was not found.', 'opace-ai-content-checker-detector' ), array( 'status' => 404 ) );
	}
	...
	public function transition( $public_id, $owner_id, $state, $receipt_id = null ) {
		...
		if ( ! isset( $transitions[ $current['state'] ] ) || ! in_array( $state, $transitions[ $current['state'] ], true ) ) {
			return new WP_Error( 'invalid_request', __( 'Unknown session state.', 'opace-ai-content-checker-detector' ) );
		}
		$table = $wpdb->prefix . 'opace_ci_jobs';
		$data  = array(
			'state'      => $state,
		...
		if ( 1 !== $updated ) {
			...
			return ! is_wp_error( $latest ) && $latest['state'] === $state ? $latest : new WP_Error( 'state_conflict', __( 'The session changed during this request. Refresh and try again.', 'opace-ai-content-checker-detector' ), array( 'status' => 409 ) );
		}
