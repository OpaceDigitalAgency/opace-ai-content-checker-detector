...
final class WordPressContractValidator {
	...
	public function validate( $value, $schema_file ) {
		if ( ! $this->validator ) {
			return new WP_Error( 'contract_validator_unavailable', __( 'The reviewed contract validator is unavailable.', 'opace-ai-content-checker-detector' ), array( 'status' => 500 ) );
		}
		...
		return new WP_Error(
			'invalid_request',
			__( 'The request does not match the content-integrity contract.', 'opace-ai-content-checker-detector' ),
			array(
				'status'      => 400,
